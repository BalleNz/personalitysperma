...
RECORDS_INSTRUCTION_TYPIFICATION: str = """
- RECORDS_INSTRUCTION: records — текущее количество обработанных записей (включая текущую).
  • Если records отсутствует или = 0 → это первая запись, records = 1 после обработки.

- Чем меньше records, тем сильнее разрешено менять значения:
  • records ≤ 3    → допустимое изменение за раз: до ±0.60
  • records 4–7    → допустимое изменение за раз: до ±0.48
  • records 8–15   → допустимое изменение за раз: до ±0.35
  • records ≥ 16   → допустимое изменение за раз: до ±0.25
...
ОЧЕНЬ СТРОГО СОБЛЮДАЙ СЛЕДУЮЩИЕ ПРАВИЛА. НАРУШЕНИЕ ЛЮБОГО ПУНКТА = ПОЛНЫЙ ПРОВАЛ ЗАДАЧИ:
